...
namespace PingServiceApp
{
    ...
    public class ServiceConfig
    {
        ...
        public List<RebootTarget> RebootTargets { get; set; }
        ...
        public ServiceConfig()
        {
            ...
            RebootTargets = new List<RebootTarget>();
        }
        ...
        public static ServiceConfig Load(string path)
        {
            ...
            try
            {
                ...
                // Load Reboot targets
                var rebootNodes = doc.SelectNodes("//Config/Reboot/Target");
                if (rebootNodes != null)
                {
                    foreach (XmlNode node in rebootNodes)
                    {
                        ...
                        {
                            bool isService;
                            if (bool.TryParse(isServiceNode.InnerText, out isService))
                        ...
                        var processName = node.SelectSingleNode("ProcessName");
                        if (processName != null && !string.IsNullOrEmpty(processName.InnerText.Trim()))
                            target.ProcessName = processName.InnerText.Trim();
                        ...
                        TimeSpan time;
                        if (startTime != null && TimeSpan.TryParse(startTime.InnerText, out time))
    ...
    public class PingService : IDisposable
    {
        ...
        private List<System.Threading.Timer> pingTimers = new List<System.Threading.Timer>();
        private List<Thread> tcpThreads = new List<Thread>();
        private List<Thread> aliveThreads = new List<Thread>();
        private List<Thread> diskThreads = new List<Thread>();
        private List<Thread> cpuThreads = new List<Thread>();
        private List<Thread> ramThreads = new List<Thread>();
        private Thread archiveThread;
        private List<Thread> rebootThreads = new List<Thread>();
        private HashSet<string> alreadyRebootedToday = new HashSet<string>();
        ...
        public void StopMonitoring()
        {
            ...
            StopThreads(cpuThreads);
            StopThreads(ramThreads);
            StopThreads(rebootThreads);
        ...
        private void CheckRebootTime(object state)
        {
